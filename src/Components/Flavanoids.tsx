...
const Flavanoids = () => {
  ...
  const calculateClassStatistics = (data: any[]) => {
    ...
    for (const item of data) {
      const nameOfClass = item.Alcohol.toString(); // Getting class name
    ...
    for (const nameOfClass in statistics) {
      statistics[nameOfClass].mean = calculateMean(
        statistics[nameOfClass].mean
      ).toFixed(3);
      statistics[nameOfClass].median = calculateMedian(
        statistics[nameOfClass].median
      ).toFixed(3);
      statistics[nameOfClass].mode = calculateMode(
        statistics[nameOfClass].mode
      );
    }
  ...
  return (
    <div>
      ...
      <table className="custom-table">
        ...
        <tbody>
          {Object.entries(classStatistics).map(
            ([nameOfClass, stats]: [string, any]) => (
              <tr key={nameOfClass}>
