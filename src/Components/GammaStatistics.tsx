...
const GammaStatistics = () => {
  ...
  const calculateClassStatistics = (data: any) => {
    ...
    for (const item of data) {
      const className = item.Alcohol.toString(); // Getting class name
    ...
    for (const className in statistics) {
      statistics[className].mean = calculateMean(
        statistics[className].gamma
      ).toFixed(3);
      statistics[className].median = calculateMedian(
        statistics[className].gamma
      ).toFixed(3);
